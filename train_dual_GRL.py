...
import val_dual_GRL as validate  #quoccuong
from models.experimental import attempt_load
from models.yolo_GRL import Model #quoccuong
...
from utils.damain_loss import DA_loss  #quoccuong
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK != -1:
            ...
            target_loader.sampler.set_epoch(epoch) #quoccuong
        pbar = enumerate(train_loader) #quoccuong
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            try: #quoccuong
                target_imgs, _, _, _ = next(target_data) #quoccuong
            except (AttributeError, StopIteration): #quoccuong
                target_data = iter(target_loader) #quoccuong
                target_imgs, _, _, _ = next(target_data) #quoccuong
            t_imgs = target_imgs.to(device, non_blocking=True).float() / 255 #quoccuong
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
            ...
            with torch.cuda.amp.autocast(amp):
                pred, source_pred_label = model(imgs)  #quoccuong
                _, target_pred_label = model(t_imgs) #quoccuong
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/source2target.yaml', help='dataset.yaml path')
