...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            if isinstance(m,Classifyy): #quoccuong
                x = m(x)  # run #quoccuong
                pred.append(x) #quoccuong
            else: #quoccuong
                x = m(x)            #quoccuong
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, DDetect, Segment, DSegment, Panoptic)):
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))[0]])  # forward #quoccuong
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m is Classifyy:  #quoccuong
            c1=ch[f]     #quoccuong
            c2=1     #quoccuong
            args = [c1,c2]  #quoccuong
        else:
            c2 = ch[f]
